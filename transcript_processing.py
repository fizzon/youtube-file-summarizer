...
DEFAULT_REQUEST_INTERVAL_SECONDS = 60 / DEFAULT_REQUESTS_PER_MINUTE
...
def _is_rate_limit_error(error: Exception) -> bool:
    """Return True when Gemini returned a quota/rate-limit response."""
...
def _respect_request_interval(request_interval_seconds: float) -> None:
    """Wait before the next Gemini call to stay within free-tier request limits."""
    global _LAST_API_CALL_TS
    if _LAST_API_CALL_TS is None:
        return

    elapsed = time.monotonic() - _LAST_API_CALL_TS
    remaining_wait = request_interval_seconds - elapsed
    if remaining_wait > 0:
        logging.info("Waiting %.2f seconds before next Gemini request.", remaining_wait)
...
def _generate_content_with_retry(
    ...
    """Generate content with request throttling and retry on quota errors."""
    global _LAST_API_CALL_TS

    for attempt in range(retry_attempts + 1):
        _respect_request_interval(request_interval_seconds)
        try:
            ...
            _LAST_API_CALL_TS = time.monotonic()
            if not _is_rate_limit_error(error) or attempt == retry_attempts:
                raise

            retry_delay = _extract_retry_delay_seconds(error)
            if retry_delay is None:
                retry_delay = min(30.0, 2 ** (attempt + 1))

            message = (
                f"Досягнуто ліміту Gemini API (429). Спроба {attempt + 1}/{retry_attempts + 1}. "
                f"Очікування {retry_delay:.1f} c перед повтором..."
            )
            st.warning(message)
            logging.warning(message)
            time.sleep(retry_delay)

    raise RuntimeError("Unexpected retry flow while calling Gemini API.")
